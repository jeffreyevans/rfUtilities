...
#' @description Various scaling approaches for scaling permuted parameter importance values
#'              
#' @param x         randomForest or ranger object with a classification or regression instance 
...
#' 
#' ( rgr.class <- ranger(y = iris[, 5], x = iris[, -5], 
...
rf.ImpScale <- function (x, scaling=c("mir","se", "p"), n=99, sort = FALSE) { 
  if (!any(class(x) == "randomForest") && !any(class(x) == "ranger")) 
  ...
  } else if(any(class(x) == "randomForest")) {
